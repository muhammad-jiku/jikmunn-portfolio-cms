        ...
        <table className="w-full">
          <thead className="bg-zinc-50 dark:bg-zinc-950">
            <tr>
              ...
              <th className="px-3 sm:px-6 py-2 sm:py-3 text-right text-[10px] sm:text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">
                Actions
              </th>
            </tr>
          </thead>
            ...
            {skills.map((skill) => (
              <tr key={skill.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-950">
                <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-zinc-100 dark:bg-zinc-800 rounded-lg flex items-center justify-center">
                    {skill.iconUrl.startsWith('http') ? (
                      ...
                      <span className="text-xl sm:text-2xl">{skill.iconUrl}</span>
                    )}
                  </div>
                </td>
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="flex-1 bg-zinc-200 dark:bg-zinc-800 rounded-full h-1.5 sm:h-2 max-w-xs">
                      <div
                        className={`h-1.5 sm:h-2 rounded-full transition-all ${getProgressColor(skill.progress)}`}
                        style={{ width: `${skill.progress}%` }}
                      />
                    </div>
                    <span className="text-xs sm:text-sm font-medium text-zinc-700 dark:text-zinc-300 min-w-9 sm:min-w-12">
                      {skill.progress}%
                    </span>
                  </div>
                </td>
